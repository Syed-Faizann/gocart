...
export const fetchCart = createAsyncThunk(
  ...
  async ({ getToken }, thunkAPI) => {
    try {
      ...
      // Ensure cart data is in correct format
      let cartData = data.cart || {};
      
      // If cart data is in wrong format (like your API showed), fix it
      if (typeof cartData === 'object') {
        // Check for mixed format like {0: {id:..., quantity:...}, productId: 2}
        const fixedCart = {};
        
        for (const [key, value] of Object.entries(cartData)) {
          if (typeof value === 'object' && value !== null && value.id && value.quantity !== undefined) {
            // Format: {0: {id: '...', quantity: 6}}
            fixedCart[value.id] = value.quantity;
...
const cartSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchCart.fulfilled, (state, action) => {
      ...
      state.cartItems = fixedCart;
      state.total = Object.values(fixedCart).reduce(
        (acc, quantity) => acc + quantity,
        0
      );
      
      console.log('Cart loaded:', state.cartItems, 'Total:', state.total);
    });
  }
});
