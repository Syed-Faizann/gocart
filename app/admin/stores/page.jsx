...
export default function AdminStores() {
    ...
    const fetchStores = async () => {
        try {
            ...
            const {data} = await axios.get('/api/admin/stores', {headers: {Authorizatio: `Bearer ${token}`}})
    ...
    const toggleIsActive = async (storeId) => {
        try {
             const token = await getToken()
            const {data} = await axios.post('/api/admin/toggle-store', {storeId},  {headers: {Authorizatio: `Bearer ${token}`}})
            await fetchStores()
