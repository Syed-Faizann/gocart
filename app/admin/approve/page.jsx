...
export default function AdminApprove() {
    ...
    const fetchStores = async () => {
        try {
            ...
            const {data} = await axios.get('/api/admin/approved-store', {headers : {Authorization: `Bearer ${token}`}})
    ...
    const handleApprove = async ({ storeId, status }) => {
       try {
              const token = await getToken()
            const {data} = await axios.post('/api/admin/approved-store', {storeId, status}, {headers : {Authorization: `Bearer ${token}`}})
